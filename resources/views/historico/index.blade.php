...
@section('content')
    @php
        ...
        foreach ($byDay as $dayKey => $dayPresences) {
             usort($dayPresences, function($a, $b) {
                return strcmp($a->occurred_at, $b->occurred_at);
            });
            $byDay[$dayKey] = $dayPresences;
        }
        ...
        <div class="flex-1 px-4 pt-4 pb-24 max-w-md mx-auto w-full">
            ...
            <div class="space-y-2">
                @for ($d = $start->copy(); $d->lte($end); $d->addDay())
                    @php
                        $key = $d->format('Y-m-d');
                        $dayPresences = $byDay[$key] ?? [];

                        $p = $dayPresences[0] ?? null;

                        $count = count($dayPresences);
                        $isPresent = $count > 0;

                        $just = $p?->justification;
                        $justStatus = $just?->status;

                        $firstTime = $p && $p->occurred_at ? Carbon::parse($p->occurred_at)->format('H:i') : $defaultFrom;

                        if ($count > 1) {
                            $lastP = end($dayPresences);
                            $lastTime = Carbon::parse($lastP->occurred_at)->format('H:i');
                        } elseif ($count === 1) {
                            $lastTime = '--:--';
                        } else {
                            $lastTime = $defaultTo;
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col">
                                    @if ($isPresent)
                                        <div class="text-gray-900 dark:text-gray-100">{{ $firstTime }} - {{ $lastTime }}</div>
                                    @else
